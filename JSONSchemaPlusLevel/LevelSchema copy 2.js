...
export default class LevelSchema {
    ...
    constructor(Level, levelPath){
        // Set JSON Encoding in level_db_config
        this.ajv = new Ajv()
        this.levelPath = levelPath
        this.Level = Level
        this.db = new Level(levelPath, { valueEncoding: 'json' })
    }
    ...
    async putSchemaSublevel(sublevel_name, sublevel_schema){
        ...
        let db = new this.Level(levelPath, { valueEncoding: 'json' })
        let specialDB = db.sublevel("JSONSchema", { valueEncoding: 'json' })
        try {
            // We do not want to overwtite Schemas that already exist
            console.log("schemaDB")
            console.log(specialDB)
            await specialDB(sublevel_name)
        } catch (error) {
            await specialDB(sublevel_name, sublevel_schema, { valueEncoding: 'json' })
    ...
    async getJSONSchema(sublevel_name){
        try {
            let the_schema = await specialDB(sublevel_name)
            return the_schema
        } catch (error) {
            return {
                status : "error",
